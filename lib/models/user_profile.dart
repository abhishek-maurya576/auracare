...
class UserProfile {
  ...
  final int? age;
  final DateTime?
      birthDate; // Birth date for age calculation and COPPA compliance
  ...
  final Map<String, dynamic>?
      crisisProtocol; // Emergency contacts and procedures
  ...
  factory UserProfile.fromJson(Map<String, dynamic> json) {
    return UserProfile(
      ...
      preferredCopingStrategies:
          _safeListFromJson(json['preferredCopingStrategies']),
      ...
      successfulInterventions:
          _safeListFromJson(json['successfulInterventions']),
  ...
  static List<String>? _safeListFromJson(dynamic jsonList) {
    ...
    try {
      return jsonList.where((item) => item is String).cast<String>().toList();
    } catch (e) {
  ...
  UserProfile copyWith({
    ...
    return UserProfile(
      ...
      mentalHealthChallenges:
          mentalHealthChallenges ?? this.mentalHealthChallenges,
      preferredCopingStrategies:
          preferredCopingStrategies ?? this.preferredCopingStrategies,
      ...
      successfulInterventions:
          successfulInterventions ?? this.successfulInterventions,
      ...
      enableProactiveSupport:
          enableProactiveSupport ?? this.enableProactiveSupport,
  ...
  String generatePersonalizationContext() {
    ...
    context
        .writeln('USER PROFILE CONTEXT (Use this to personalize responses):');
    ...
    if (mentalHealthGoals != null && mentalHealthGoals!.isNotEmpty) {
      context
          .writeln('- Mental health goals: ${mentalHealthGoals!.join(", ")}');
    }

    // Coping strategies that work
    if (preferredCopingStrategies != null &&
        preferredCopingStrategies!.isNotEmpty) {
      context.writeln(
          '- Preferred coping strategies: ${preferredCopingStrategies!.join(", ")}');
    }

    // Known triggers
    if (triggers != null && triggers!.isNotEmpty) {
      context.writeln(
          '- Known triggers to be mindful of: ${triggers!.join(", ")}');
    }

    // Personal strengths
    if (strengths != null && strengths!.isNotEmpty) {
      context.writeln(
          '- Personal strengths to reinforce: ${strengths!.join(", ")}');
    }
    ...
    if (successfulInterventions != null &&
        successfulInterventions!.isNotEmpty) {
      context.writeln(
          '- Previously successful interventions: ${successfulInterventions!.join(", ")}');
    }
    ...
    context.writeln(
        '- Conversation depth preference: $conversationDepth/5 (1=surface level, 5=very deep)');
  ...
  String generateMoodContext() {
    ...
    if (stressPatterns != null) {
      if (stressPatterns!.containsKey('averageStress')) {
        context.writeln(
            '- Average stress level: ${stressPatterns!['averageStress']}/10');
      }
