...
class UserProfileService {
  ...
  Future<void> saveUserProfile(UserProfile profile) async {
    try {
      ...
      debugPrint('User profile saved and updated in Gemini service: ${profile.name}');
    } catch (e) {
  ...
  Future<void> updateUserProfile(String userId, Map<String, dynamic> updates) async {
    try {
      ...
      // Reload the profile to update Gemini service
      await loadUserProfile(userId);
      
      debugPrint('User profile updated for user: $userId');
    } catch (e) {
  ...
  Future<void> updateMoodPatterns(String userId, List<MoodEntry> moodEntries) async {
    try {
  ...
  Map<String, dynamic> _calculateMoodPatterns(List<MoodEntry> entries) {
    ...
    // Calculate averages
    final avgMoodScore = entries.fold<double>(0, (total, entry) => total + entry.moodScore) / entries.length;
  ...
  Map<String, dynamic> _calculateStressPatterns(List<MoodEntry> entries) {
    ...
    final avgStress = entries.fold<double>(0, (total, entry) => total + entry.calculatedStressLevel) / entries.length;
    final crisisCount = entries.where((entry) => entry.isCrisisLevel).length;

    // Determine stress trend
    String trend = 'stable';
    if (entries.length >= 3) {
      final recent = entries.take(3).map((e) => e.calculatedStressLevel).toList();
      final recentAvg = recent.reduce((a, b) => a + b) / recent.length;
      final previous = entries.skip(3).take(3).map((e) => e.calculatedStressLevel).toList();
  ...
  Future<void> setupPersonalizationPreferences(String userId, {
    ...
    try {
      ...
      if (mentalHealthGoals != null) updates['mentalHealthGoals'] = mentalHealthGoals;
      if (preferredCopingStrategies != null) updates['preferredCopingStrategies'] = preferredCopingStrategies;
      if (communicationStyle != null) updates['communicationStyle'] = communicationStyle;
      if (triggers != null) updates['triggers'] = triggers;
      if (strengths != null) updates['strengths'] = strengths;
      if (interests != null) updates['interests'] = interests;
      if (conversationDepth != null) updates['conversationDepth'] = conversationDepth;
      if (enablePersonalization != null) updates['enablePersonalization'] = enablePersonalization;
      if (shareEmotionalState != null) updates['shareEmotionalState'] = shareEmotionalState;
  ...
  Future<void> updateSuccessfulInterventions(String userId, List<String> interventions) async {
    try {
      await updateUserProfile(userId, {
  ...
  Future<void> updateCrisisProtocol(String userId, Map<String, dynamic> crisisInfo) async {
    try {
      await updateUserProfile(userId, {
